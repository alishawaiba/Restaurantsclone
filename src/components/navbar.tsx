    ...
    <>
      <header className="sticky top-0 z-50">
        <nav className="bg-gray-600 text-white p-4 flex justify-between items-center h-16 w-full mx-auto">
          ...
          >
            <img
              src={logo}
              alt="TAOR Logo"
              className="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 lg:h-14 lg:w-14 object-contain transition-transform duration-200 hover:scale-105"
            />
        ...
        <nav className="bg-gray-800 w-full p-3 text-white shadow-lg hidden sm:block">
          <div className="flex flex-wrap items-center space-x-6 max-w-7xl ">
            {navLinks.map((link) => (
              <button
                key={link.section}
